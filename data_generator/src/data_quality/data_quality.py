...
from utils import sample_func,print_colored
...
#from IPython.display import HTML # For rendering HTML in notebooks, or just for string output
...
class data_quality:
    ...
    def load_data(self):
        ...
        for table_name, table_info in self.config.SCHEMA.items():
            ...
            try:
                ...
                print(f"  {table_name}: Error loading data - {e}")
        ...
        if not all(table in self.data for table in required_tables):
            print("  CRITICAL: Not all required tables could be loaded. Aborting further checks.")
    ...
    def validate_referential_integrity(self):
        ...
        """
        #print_colored("\n--- Running Referential Integrity Checks ---", Colors.HEADER)
    ...
    def validate_completeness(self):
        ...
        """
        print("\n--- Running Completeness Checks ---")#, Colors.HEADER)
        ...
        for table_name, table_info in self.config.SCHEMA.items():
            ...
            df = self.data[table_name]
            for field_name, field_props in table_info["fields"].items():
                if field_props["type"] == datetime.date and field_name in df.columns and field_name not in ['VALID_TO','EINDT','BUDAT']:
    ...
    def calculate_overall_dq_score(self):
        ...
        print(f"\nOverall Data Quality Score: {self.overall_dq_score:.2f}/100")
