...
local_css("dashboard\style.css") # Assuming style.css is in the same directory
...
if page == "Executive Overview":
    ...
    with col1:
        st.metric(label=f"**Total Spend**", value=f"${(current_spend/1000000000):,.2f}B", delta=spend_trend_val, delta_color=spend_trend_status)
    ...
    with col1:
        ...
        fig_monthly_spend = px.line(monthly_spend_filtered, x='MONTH', y='SPEND', title='Monthly Spend in Millions USD',
                                    color_discrete_sequence=[COLORS["primary"]])
        fig_monthly_spend.update_layout(hovermode="x unified",margin=dict(l=30, r=30, t=50, b=50))
        st.plotly_chart(fig_monthly_spend, use_container_width=True)
    
    with col2:
        st.subheader("Spend by Category ")
        fig_spend_category = px.pie(data["spend_by_category"], values='TOTAL_SPEND', names='CATEGORY', title='Spend by Category in Millions USD',
                                   color_discrete_sequence=px.colors.sequential.RdBu) # Example color sequence
        fig_spend_category.update_layout(
        legend=dict(
        ...
        margin=dict(l=25, r=25, t=35, b=25), # Adjust plot margins
        title_x=0 ,# Centers the main title horizontally
        title_y=0.95
)   
        fig_spend_category.update_traces(
        domain={'x': [0.1, 0.9], 'y': [0.1, .9]} # Adjust the X domain to be narrower
)
    ...
    top_vendors.rename(columns={'NAME1': 'Vendor Name','TOTAL_SPEND':'Total Spend','TOTAL_SPEND_PERCENT':'Total Spend Percent','ON_TIME_DELIVERY_RATE':'On time Delivery Rate'}, inplace=True)
    st.dataframe(top_vendors[['Vendor Name', 'Total Spend', 'Total Spend Percent', 'On time Delivery Rate']].style.format({
        'Total Spend': "${:,.2f}",
        'Total Spend Percent': "{:.2%}",
...
# --- Page 2: Vendor Intelligence ---
    ...
    filtered_vendors.rename(columns={'NAME1': 'Vendor Name','LAND1' :"Country Code",'TOTAL_SPEND':'Total Spend','NUM_POS':'No of Orders',
                                     'ON_TIME_DELIVERY_RATE':'On time Delivery Rate', 'PRICE_COMPETITIVENESS_SCORE': 'Price Competitiveness Score',
                                     'RISK_LEVEL': 'Risk Level'}, inplace=True)
    display_cols = ['Vendor Name', 'Country Code', 'Total Spend', 'No of Orders', 'On time Delivery Rate', 'Price Competitiveness Score', 'Risk Level']
    st.dataframe(filtered_vendors[display_cols].style.format({
        'Total Spend': "${:,.2f}",
        'On time Delivery Rate': "{:.1%}",
        'Price Competitiveness Score': "{:.0f}"
    }), use_container_width=True,hide_index=True)

    # --- Selected Vendor Detail View ---
    st.subheader("Selected Vendor Detail")
    selected_vendor_name = st.selectbox("Select a Vendor for Details", filtered_vendors['Vendor Name'].unique(),)

    colV_1, colV_2, colV_3 = st.columns(3)
    if selected_vendor_name:
        selected_vendor_data = filtered_vendors[filtered_vendors['Vendor Name'] == selected_vendor_name].iloc[0]
        with colV_1:
            st.write(f"**Vendor Name:** {selected_vendor_data['Vendor Name']}")
        with colV_2:
            st.write(f"**Vendor ID:** {selected_vendor_data['LIFNR']}")
            st.write(f"**Country:** {selected_vendor_data['Country Code']}")
        with colV_3:
        ...
        with col_det1:
            ...
            vendor_spend_trend.rename(columns={'AEDAT': 'Date','TOTAL_SPEND':"Total Spend"}, inplace=True)
            fig_vendor_spend_trend = px.line(vendor_spend_trend, x='Date', y='Total Spend', title=f'Spend Trend for {selected_vendor_name}',
                                            color_discrete_sequence=[COLORS["primary"]])
            fig_vendor_spend_trend.update_layout(hovermode="x unified",margin=dict(l=30, r=30, t=50, b=50))
            st.plotly_chart(fig_vendor_spend_trend, use_container_width=True)
        
        with col_det2:
            st.write("#### Performance Metrics")
            df_details=pd.DataFrame({
                "KPI": ["Total Spend", "No of POs", "On-Time Delivery Rate", "Price Competitiveness Score", "Risk Level"],
                "Value": [
                    f"${selected_vendor_data['Total Spend']:,.2f}",
                    f"{selected_vendor_data['No of Orders']}",
                    f"{selected_vendor_data['On time Delivery Rate']:.1%}",
                    f"{selected_vendor_data['Price Competitiveness Score']:.0f}",
                    selected_vendor_data['Risk Level']
                ]
            })
            

            st.table(df_details.style.hide(axis="index"),border="horizontal")

        st.write("#### Top ten Materials Supplied")
        ...
        top_materials.rename(columns={'MAKTX': 'Material', 'TOTAL_SPEND': 'Total Spend'}, inplace=True)
        st.dataframe(top_materials.style.format({'Total Spend': "${:,.2f}"}), use_container_width=True,hide_index=True)
        ...
        recent_transactions.rename(columns={'EBELN': 'PO Number', 'AEDAT': 'Date', 'MATNR': 'Material Number', 'MAKTX': 'Material Description',
                                            'MENGE': 'Quantity', 'NETPR': 'Net Price', 'TOTAL_SPEND': 'Total Spend'}, inplace=True)
        st.dataframe(recent_transactions[['PO Number', 'Date', 'Material Number', 'Material Description', 'Quantity', 'Net Price', 'Total Spend']].style.format({
            'Total Spend': "${:,.2f}", 'Net Price': "${:,.2f}"
        }), use_container_width=True,hide_index=True)
    ...
    with col_chart1:
        ...
        fig_spend_perf = px.scatter(filtered_vendors, x='Total Spend', y='On time Delivery Rate',
                                    hover_name='Vendor Name', size='Total Spend', color='Risk Level',
    ...
    with col_chart2:
        ...
        fig_delivery_dist = px.histogram(filtered_vendors, x='On time Delivery Rate', nbins=10,
    ...
    spend_bins = pd.qcut(filtered_vendors['Total Spend'], q=2, labels=['Low Spend', 'High Spend'])
    performance_bins = pd.qcut(filtered_vendors['On time Delivery Rate'], q=2, labels=['Low Performance', 'High Performance'],duplicates='drop')
...
# --- Page 3: Savings & Opportunities ---
    ...
    total_savings_potential=total_savings_potential/1000000  # Convert to millions

    num_opportunities = len([s for s in data["savings_opportunities"].values() if s > 0])
    estimated_roi = 0.15 # Placeholder

    with col1:
        st.metric(label="**Total Savings Potential**", value=f"${total_savings_potential:,.2f} M")
    with col2:
    ...
    fig_maverick.update_layout(margin=dict(l=30, r=30, t=50, b=50))
    ...
    st.dataframe(price_variance_table, use_container_width=True,hide_index=True)
    ...
    st.dataframe(consolidation_table, use_container_width=True,hide_index=True)
    ...
    st.dataframe(contract_gaps_table, use_container_width=True,hide_index=True)
...
# --- Page 4: Material & Category Analysis ---
    ...
    fig_cat_spend = px.bar(data["spend_by_category"], x='CATEGORY', y='TOTAL_SPEND', title='Spend by Category',
                           color_discrete_sequence=[COLORS["primary"]])
    fig_cat_spend.update_layout(margin=dict(l=30, r=30, t=50, b=50))
    ...
    st.dataframe(category_scorecard, use_container_width=True,hide_index=True)
    ...
    st.dataframe(filtered_materials[['MATNR', 'MAKTX', 'MATKL', 'MTART', 'MEINS']], use_container_width=True,hide_index=True)
    ...
    if selected_matnr:
        ...
        with colM1:
            st.write(f"#### Material ID: {selected_matnr}")
        ...
        with col_mat_det1:
            ...
            fig_price_history = px.line(price_history, x='AEDAT', y='NETPR', title='Average Price Over Time',
                                        color_discrete_sequence=[COLORS["primary"]])
            fig_price_history.update_layout(hovermode="x unified",margin=dict(l=30, r=30, t=50, b=50))
